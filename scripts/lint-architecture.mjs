...
export const rules = [
  {
    ...
    message: "Use mobile-first min-width breakpoints; max-width and width< queries invert the layout contract.",
    pattern: /@media\s*\([^)]*(?:max-width\s*:|width\s*(?:<|<=))/giu,
  },
  {
    id: "responsive/no-px-breakpoint",
    message: "Express breakpoints in rem/em so browser text scaling participates in responsive decisions.",
  ...
  },
  {
    ...
    message: "Do not use 100vh for application layout; use dynamic/small viewport units where viewport height is intentional.",
    pattern: /\b(?:height|min-height|max-height|block-size|min-block-size|max-block-size)\s*:\s*(?:calc\([^;]*\b100vh\b[^;]*\)|100vh)\s*;/giu,
  },
  {
    id: "responsive/no-horizontal-overflow-mask",
    message: "Do not hide or clip horizontal overflow to conceal layout defects; fix the overflowing descendant.",
  ...
  },
  {
    ...
    message: "Use rem/em or another scalable unit for font-size; pixel font sizes resist user scaling conventions.",
  ...
  },
  {
    ...
    message: "Transition explicit properties only; transition: all creates accidental motion and performance regressions.",
  ...
  for (const [rule, files] of Object.entries(baseline)) {
    for (const [file, expected] of Object.entries(files)) {
      ...
      if (actual > expected) {
        ...
        problems.push(`${key}: debt decreased from ${expected} to ${actual}; lower the baseline in config/responsive-lint-baseline.json`);
      }
    }
  }
...
export async function run(root = process.cwd()) {
  ...
  console.log(`architecture-lint: passed (${debt} ratcheted legacy finding${debt === 1 ? "" : "s"})`);
}
